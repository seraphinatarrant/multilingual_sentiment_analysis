...
Then uses them to calculate performance metrics and write them out.

If 'save' is set, writes out a _polarity csv with gold label included and also predictions converted to polarity
...
from utils.data_utils import get_label_from_emotion, convert_to_polarity, polarity_string2int
...
def setup_argparse():
    ...
    p.add_argument('--skip_perf_eval', action='store_true', help='skips doing evaluation, just create new csvs')
    return p.parse_args()
